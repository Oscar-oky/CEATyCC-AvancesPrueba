...
const Calendar: React.FC<CalendarProps> = () => {
  ...
  const handleDayClick = (day: number) => {
    ...
    if (dayEvents.length === 0 && isLoggedIn()) {
      // No hay eventos y está logueado, verificar si es admin para crear
      if (isAdmin()) {
        setSelectedDateForEvent(clickedDate);
        setSelectedEvent(null);
        setIsModalOpen(true);
      }
    } else if (dayEvents.length === 0 && !isLoggedIn()) {
  ...
  const handleLogin = (email: string, password?: string) => {
    ...
    // Si había una fecha seleccionada y es admin, abrir modal de evento
    if (selectedDateForEvent && isAdmin()) {
      setIsModalOpen(true);
    }
  };
  ...
  return (
    <div className="w-full sm:w-72 lg:w-80 flex-shrink-0">
      <div className="bg-gray-800 text-white p-3 sm:p-4 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {daysOfWeek.map((day, index) => (
            <div key={index} className="text-center text-gray-300 font-bold text-sm p-2">
              {day}
            </div>
          ))}
        </div>
                ...
                <div className="mt-2 space-y-1">
                  {dayEvents.map(event => (
                    ...
                    >
                      ...
                      {event.location && (
                        <div className="opacity-75 text-xs">📍 {event.location}</div>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {/* User Status */}
        {isLoggedIn() && (
          <div className="text-center mb-4 p-2 bg-blue-700 rounded">
