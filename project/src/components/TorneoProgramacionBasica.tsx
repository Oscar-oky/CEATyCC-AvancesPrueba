...
const TorneoProgramacionBasica: React.FC = () => {
  ...
  const [torneoBasicoImages, setTorneoBasicoImages] = useState<string[]>([
  ]);
    ...
    Array.from(files).forEach(file => {
      if (file.type.startsWith('image/')) {
        ...
        reader.onload = (e) => {
          ...
          setTorneoBasicoImages(prev => [...prev, imageUrl]);
        };
  ...
  const handleDeleteImage = (index: number) => {
    setTorneoBasicoImages(prev => prev.filter((_, i) => i !== index));
  };
            ...
            {isAdmin && (
              <div className="space-y-3">
                ...
                {torneoBasicoImages.length > 0 && (
                  <div>
                    <h5 className="text-sm font-medium text-gray-700 mb-2">Imágenes actuales ({torneoBasicoImages.length}):</h5>
